...
public class SegmentedButton extends View
{
    ...
    private void getAttributes(Context context, @Nullable AttributeSet attrs)
    {
        ...
        if (ta.hasValue(R.styleable.SegmentedButton_segmentedButton_selectedBackground))
            selectedBackgroundDrawable = ta.getDrawable(R.styleable.SegmentedButton_segmentedButton_selectedBackground);

        rounded = ta.getBoolean(R.styleable.SegmentedButton_segmentedButton_rounded, false);

        // Parse ripple color value and update the ripple
        setRipple(ta.getColor(R.styleable.SegmentedButton_segmentedButton_rippleColor, Color.GRAY));

        // Load drawable if available, otherwise variable will be null
        if (ta.hasValue(R.styleable.SegmentedButton_segmentedButton_drawable))
        {
            int drawableResId = ta.getResourceId(R.styleable.SegmentedButton_segmentedButton_drawable, -1);
            drawable = readCompatDrawable(context, drawableResId);
        }
        drawablePadding = ta.getDimensionPixelSize(R.styleable.SegmentedButton_segmentedButton_drawablePadding, 0);
        hasDrawableTint = ta.hasValue(R.styleable.SegmentedButton_segmentedButton_drawableTint);
        drawableTint = ta.getColor(R.styleable.SegmentedButton_segmentedButton_drawableTint, -1);
        hasSelectedDrawableTint = ta.hasValue(R.styleable.SegmentedButton_segmentedButton_selectedDrawableTint);
        selectedDrawableTint = ta.getColor(R.styleable.SegmentedButton_segmentedButton_selectedDrawableTint, -1);
        hasDrawableWidth = ta.hasValue(R.styleable.SegmentedButton_segmentedButton_drawableWidth);
        hasDrawableHeight = ta.hasValue(R.styleable.SegmentedButton_segmentedButton_drawableHeight);
        drawableWidth = ta.getDimensionPixelSize(R.styleable.SegmentedButton_segmentedButton_drawableWidth, -1);
        drawableHeight = ta.getDimensionPixelSize(R.styleable.SegmentedButton_segmentedButton_drawableHeight, -1);
        drawableGravity = ta.getInteger(R.styleable.SegmentedButton_segmentedButton_drawableGravity, Gravity.LEFT);

        hasText = ta.hasValue(R.styleable.SegmentedButton_segmentedButton_text);
        text = ta.getString(R.styleable.SegmentedButton_segmentedButton_text);
        textColor = ta.getColor(R.styleable.SegmentedButton_segmentedButton_textColor, Color.GRAY);
        hasSelectedTextColor = ta.hasValue(R.styleable.SegmentedButton_segmentedButton_selectedTextColor);
        selectedTextColor = ta.getColor(R.styleable.SegmentedButton_segmentedButton_selectedTextColor, Color.WHITE);
        linesCount = ta.getInt(R.styleable.SegmentedButton_segmentedButton_linesCount, Integer.MAX_VALUE);
        ...
        textSize = ta.getDimension(R.styleable.SegmentedButton_segmentedButton_textSize, px14sp);

        final boolean hasFontFamily = ta.hasValue(R.styleable.SegmentedButton_android_fontFamily);
        final int textStyle = ta.getInt(R.styleable.SegmentedButton_segmentedButton_textStyle, Typeface.NORMAL);
        final int selectedTextStyle = ta.getInt(R.styleable.SegmentedButton_segmentedButton_selectedTextStyle, textStyle);
